...
# please do not deviate from these specifications.
        ...
        if idx < len(self):
            input_data = torch.tensor(data[idx, :5], dtype=torch.float32)
            label = torch.tensor(data[idx, 5], dtype=torch.float32)
...
def main():
    ...
    for idx, sample in enumerate(data_loaders.train_loader):
        ...
        # print ("train sample input = ", sample['input'], "train sample label = ", sample['label'])
    for idx, sample in enumerate(data_loaders.test_loader):
        _, _ = sample['input'], sample['label']
        # print ("test sample input = ", sample['input'], "test sample label = ", sample['label'])
